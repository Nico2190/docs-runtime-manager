...
You can route external requests to `+https://app.example.com+` to `+http://app.cloudhub.io/api+`:
...
Alternatively, you can define other patterns to hold the input value:

[%header,cols="10a,20a,20a,20a,10a"]
|===
| Rule # | *Input Path* | *Target App* | *Output Path* | *Protocol*
|1 | /{app}/{env} | {app}-{env} | /api | http
|===

In this case, a call to https://example.com/booking/dev would call your app called booking-dev/api 
